...
SELECT JOB_ID, COUNT(*) EMPLOYEE
FROM EMPLOYEES
GROUP BY JOB_ID;

-- 2) 동일한 월급을 받는 직원들의 월급과 직원 수를 조회
SELECT SALARY, COUNT(*) EMPLOYEE
FROM EMPLOYEES 
GROUP BY SALARY; 

-- 3) 부서별로 부서아이디, 소속직원수, 월급합계를 조회
--     단, 부서ID가 NULL인 것은 제외
SELECT DPARTMENT_ID, COUNT(*)EMPLOYEE, SUM(SALARY)
FROM EMPLOYEES 
...
SELECT DEPARTMENT_ID, TO_CHAR(HIRE_DATE,'YYYY'), 
	COUNT(EMPLOYEE_ID)
FROM EMPLOYEES 
GROUP BY DEPARTMENT_ID,TO_CHAR(HIRE_DATE,'YYYY')----고용연도 뽑는법
...
SELECT COUNT(EMPLOYEE_ID), LOCATION_ID, DEPARTMENT_ID, JOB_ID 
...
--	단, 지역번호는 앞 3자리로 함
SELECT SUBSTR(PHOME_NUM)
FROM EMPLOYEES 
GROUP BY PHONE_NUMBER
HAVING LOCATION_NUMBER---------------------????

-- 15) 부서별 연간총급여((월급+월급*커미션퍼센트)*12)를 산출하고
--       연간총급여가 높은 순으로 부서아이디와 연간총급여를 조회
--       단, 부서가 정해지지 않은 경우는 제외
SELECT DEPARTMENT_ID, SUM((SALARY + SALARY*NVL(COMMISSION_PCT,0))*12)TOT, 
FROM DEPARTMENTS 
WHERE DEPARTMENT_ID IS NOT NULL 
GROUP BY DEPARTMENT_ID 
HAVING TOTAL SEDC;
...
SELECT COUNTRY_ID, LOCATION_ID, DEPARTMENT_ID 
