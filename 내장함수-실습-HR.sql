...
SELECT REPLACE(EMPLOYYE_PONE NUMBER, '.','');
FROM EMPLOYEES;
...
SELECT LAST_NAME
	SUBSTR(LAST_NAME,0,INSERT(LAST_NAME,'a')-1) --첫번째 a 나오기 전
	||SUBSTR(LAST_NAME,0,INSERT(LAST_NAME,'a')+1)-- 첫번째 a 나온 후
FROM EMPLOYEES;

-- 5) 모든 직원들의 이메일주소를 20자리에 맞춰 좌측을 -기호로 채워 조회
SELECT EMPLOYEES_EMAIL 
SELECT LPAD('ABCDE',10)FROM DUAL;

SELECT LPAD(EMPLOYEE_EMAIL, 20, '-') 
FROM EMPLOYEES;
...
SELECT 
    EMPLOYEE_ID,
    SALARY,
    CEIL(SALARY / (30 * 8 * 60)) AS CEIL_SALARY_PER_MINUTE,
    FLOOR(SALARY / (30 * 8 * 60)) AS FLOOR_SALARY_PER_MINUTE
FROM EMPLOYEES;


SELECT CEIL(SALARY/30/8/60), FLOOR(SALARY/30/8/60)
FROM EMPLOYEES; 
...
SELECT 
ADD_MONTHS(SYSDATE, 3),NEXT_DAY(SYSDATE,'토요일')FROM DUAL;

SELECT NEXT_DAY(ADD_MONTHS(SYSDATE,3),'토') 
FROM DUAL;

--    날짜/시간 정보를 조회
SELECT TO_DATE('2024-12-11')FROM DUAL;
...
SELECT EMPLOYEE_ID, 
	FIRST_NAME||' '||LAST_NAME AS FULL_NANE,
	SALARY
FROM EMPLOYEES
WHERE MANAGER_ID = 149;  --WHERE 는 FROM 다음에 와야 한다
...
AND MOD(TO_CHAR(HIRE_DATE,'YYYY'),2)=0;
...
-- 18) 직원들의 JOB_ID를 AC는 ACC로 ST는 STT로 변경해서 조회
 SELECT REPLACE('JOB_ID', 'AC', 'ACC')FROM DUAL;---------------????

 SELECT REPLACE(REPLACE(JOB_ID, 'AC','ACC'),'ST','STT')
 FROM EMPLOYEES; 

-- 19) 직원들의 직무시작일은 한달 전으로 직무종료일은 한달 후로 변경해 조회
 SELECT ADD_MONTHS(HIRE_DATE, -1)FROM DUAL;------?

 SELECT ADD_MONTHS(START_DATE,-1),ADD_MONTHS(END_DATE,1) 
 FROM JOB_HISTORY; 
...
SELECT 
	DECODE(JOB_ID,'IT_PROG','정보부','AC_ACCOUNT','회계부','AC_MRG','관리부')
FROM EMPLOYEES 
